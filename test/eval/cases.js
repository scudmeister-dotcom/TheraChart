...
/* ---------- grounding checks ----------
   ...
   These used to live here, where they graded the model but never guarded a
   real note. They now live in parser.js and run at request time as well, so
   the review screen can mark an ungrounded row for the therapist. Imported
   rather than re-declared: two copies of a hallucination checker is how one
   of them quietly gets better than the other.

   Read them there — each carries the reasoning for what it does and does not
   grade (why laterality is judged on contradiction rather than absence, why
   prose is checked on regions and numbers rather than word overlap, and why
   the Treatment field gets a shape test of its own). */
const PR = require("../../parser.js");
const {
  groundedInTranscript, inventedRegions, contradictedLaterality, inventedNumbers,
  treatmentIsAPlan, treatmentWithoutOne, dialogueNotInflated,
} = PR;
...
/** Which section(s) mention `re` — used to prove a sentence landed in exactly
    ...
    stylistic one. */
const SECTION_KEYS = PR.SECTION_KEYS;   // shared with the runtime checks
